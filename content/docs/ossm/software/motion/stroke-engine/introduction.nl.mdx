...
description: "Creëer veelzijdige strijkbewegingen met stappen- of servomotoren op ESP32 voor doe-het-zelf lineaire positieaandrijfmachines"
---

StrokeEngine is een bibliotheek voor het creëren van een verscheidenheid aan strijkbewegingen met stappen- of servomotoren op een ESP32. Je kunt het gebruiken met elke doe-het-zelfmachine met een lineaire positieaandrijving, aangedreven door een stappen- of servomotor.

<CardGroup cols={2}>
  <Card title="FuckIO-voorbeeld" icon="code" href="https://github.com/theelims/FuckIO">
...
<Frame caption="StrokeEngine-coördinatensysteem dat de fysieke verplaatsing, uithaalgrenzen en slagparameters weergeeft">
  <img src="./_images/coordinates.svg" alt="Coördinatiesysteemdiagram dat de relatie toont tussen fysieke reis-, keepout-grens-, diepte- en slagparameters" />
</Frame>
...
|------|-------------|
| **fysiekeReizen** | Het echte fysieke reizen van de ene harde eindstop naar de andere |
| **buitengrens** | Veiligheidsafstand van elke kant afgetrokken om botsingen te voorkomen |
| **_reis** | Werkafstand: `physicalTravel - (2 * keepoutBoundary)` |
| **Thuis** | Positie bij `-keepoutBoundary` (meestal aan de achterkant) |
| **MIN = 0** | Nulpositie, `keepoutBoundary` weg van huis |
| **Diepte** | Het verste punt dat de machine uitschuift (instelbaar tijdens runtime) |
| **Hartinfarct** | De werkafstand van een strijkbeweging (tijdens de looptijd instelbaar) |

<Note>
Beschouw **Slag** als de amplitude en **Diepte** als een lineaire offset die eraan wordt toegevoegd. De positieve bewegingsrichting is naar voren (naar het lichaam).
</Note>

### Patronen

StrokeEngine maakt gebruik van een patroongenerator om een ​​grote verscheidenheid aan sensaties te bieden. Patronen passen dynamisch parameters zoals snelheid, slag en diepte bewegingsgewijze aan met behulp van trapeziumvormige bewegingsprofielen.

Elk patroon accepteert vier parameters:
- **diepte** — Maximaal uitbreidingspunt
- **slag** — Bewegingsamplitude
- **snelheid** — Cycli per minuut
- **sensatie** — Willekeurige modificator voor patroongedrag (-100 tot 100)

<Info>
Zie de [Patroondocumentatie](/ossm/software/motion/stroke-engine/pattern) voor gedetailleerde beschrijvingen van beschikbare patronen en instructies voor het maken van uw eigen patronen.
</Info>

### Gracieuze foutafhandeling

StrokeEngine verwerkt ongeldige parameters op een elegante manier zonder de werking te onderbreken:

- Alle instelfuncties gebruiken `constrain()` om de invoer van de fysieke mogelijkheden van de machine te beperken
- Waarden buiten de grenzen worden automatisch bijgesneden
- Patroonopdrachten die de machinelimieten overschrijden, resulteren in kortere slagen of aangepaste hellingen
- De beweging wordt over de volledige afstand voltooid, maar kan iets langer duren dan verwacht

### Updates van parameters halverwege de slag

U kunt de diepte-, lijn-, snelheid- en patroonparameters halverwege de slag bijwerken voor een responsieve, vloeiende gebruikerservaring. Ingebouwde beveiligingen zorgen ervoor dat de machine altijd binnen de perken blijft.
...
| Staat | Beschrijving |
|-------|-------------|
| **ONGEDEFINIEERD** | Initiële staat vóór thuiskomst. Motor is uitgeschakeld en positie is onbekend. |
| **KLAAR** | Thuiskomen voltooid. Machine accepteert bewegingsopdrachten. |
| **PATROON** | Patroongenerator voert cyclische bewegingen uit. |
| **INSTELLINGDIEPTE** | Motor volgt de dieptepositie voor interactieve aanpassing. |
...
<Warning>
Handmatige homing is gevaarlijk. Het aanroepen van `thisIsHome()` wanneer u niet bij de fysieke eindstop bent, veroorzaakt een onjuist coördinatensysteem, wat resulteert in een crash die uw machine kan beschadigen.
</Warning>
...
2. Bel `Stroker.thisIsHome()`

```cpp
Stroker.thisIsHome();
```

Dit schakelt de bestuurder in, stelt de huidige positie in als `-keepoutBoundary` en beweegt langzaam naar positie 0.
...
|----------|-------------|
| `Stroker.startPattern()` | Start de patroongebaseerde strijkbeweging |
| `Stroker.stopMotion()` | Stop onmiddellijk met maximale vertraging |

### Positiecommando's

Ga naar een van beide uiteinden van de machine voor installatiedoeleinden:
...
<Note>
Deze functies kunnen worden opgeroepen vanuit de status `PATTERN` of `READY` en stoppen elke huidige beweging. Ze retourneren `false` als ze in een ongeldige status worden aangeroepen.
</Note>
...
Update de dieptepositie met `Stroker.setDepth(float)` en lees de huidige waarde af met `Stroker.getDepth()`.

<Tip>
**Fancy-modus:** Roep `Stroker.setupDepth(10.0, true)` op om zowel de diepte als de slag interactief aan te passen. De sensatieschuifregelaar wordt toegewezen aan het interval `[depth-stroke, depth]`:
- `sensation = 100` — Past de dieptepositie aan
- `sensation = -100` — Past de slagpositie aan
- `sensation = 0` — Posities op het middelpunt van de slag
</Tip>

### Parameterfuncties

Update parameters op elk gewenst moment. Waarden worden automatisch beperkt tot machinelimieten:
...
### Terugbellen via telemetrie

Registreer een terugbelactie om telemetriegegevens te ontvangen voor elke trapeziumvormige beweging:
...
<Note>
Raadpleeg [StrokeEngine.h](https://github.com/theelims/StrokeEngine/blob/main/src/StrokeEngine.h) in de bronrepository voor de volledige API-referentie, inclusief overbelaste functies en extra functies.
</Note>

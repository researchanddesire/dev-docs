...
Les caractéristiques sont organisées par [plages d'espaces de noms](#structure-de-lespace-de-noms-uuid) pour faciliter l'expansion et la découverte.
...
| But | Envoyer des commandes pour contrôler le comportement de l'OSSM |
...
| ------- | --------- | ----------- | ----------- |
| `set:speed:<value>` | speed | 0-100 | Définir le pourcentage de vitesse de course |
| `set:stroke:<value>` | stroke | 0-100 | Définir le pourcentage de longueur de course |
| `set:depth:<value>` | depth | 0-100 | Définir le pourcentage de profondeur de pénétration |
| `set:sensation:<value>` | sensation | 0-100 | Définir le pourcentage d'intensité de sensation |
| `set:pattern:<value>` | pattern | 0-6 | Définir le motif de course (voir [motifs](#caractéristiques-des-informations-sur-les-motifs)) |
...
| `stream:<pos>:<time>` | pos, time | pos: 0-100, time: ms | Envoyer une commande de position en mode streaming (expérimental) |
...
Lorsque la valeur est définie sur `false`, les commandes de vitesse BLE (0-100) sont utilisées directement comme valeur de vitesse, ignorant la position du bouton.

**Exemple** : commande BLE `set:speed:80` → Vitesse effective = 80 %

<Warning>
En mode indépendant, les commandes BLE peuvent contourner la limite imposée par le bouton physique. Assurez-vous que votre application met en œuvre des contrôles de sécurité appropriés.
</Warning>
</Tab>
</Tabs>
...
<Tabs>
<Tab title="Activé">
Lorsqu'il est défini sur `true`, l'OSSM s'attend à recevoir les commandes et les temps exacts du funscript, et à ce que les commandes soient envoyées
avec le même délai entre les commandes que la valeur de temps. En calculant le temps entre les commandes reçues et la variable de temps,
l'OSSM peut déterminer la latence introduite par BLE et la corriger en modifiant légèrement la vitesse du mouvement.
Si le temps entre les commandes ne correspond pas à la variable intime, cette option ne doit pas être activée. Le réglage du tampon sert à
ajouter artificiellement un délai à tous les mouvements. L'OSSM dispose ainsi du temps nécessaire pour recevoir la commande suivante avant la fin de la précédente.
Cela permet de supprimer les retards introduits par les commandes tardives et de lisser le mouvement en combinant les mouvements dans la même direction.
Les lecteurs Funscript doivent ajouter cette valeur de tampon à leur décalage de lecture et prévoir un réglage supplémentaire pour affiner ce décalage,
afin de tenir compte du temps de transmission et de tout retard inhérent au funscript lui-même.
</Tab>

<Tab title="Désactivé">
Lorsqu'il est défini sur `false`, l'OSSM exécute les commandes dès leur réception. Les commandes arrivées en retard sont exécutées en retard.
Les commandes qui arrivent tôt sont exécutées lorsque la commande précédente est terminée.
...
| `fail:wifi:invalid_format` | Format de commande incorrect (délimiteur pipe (\|) manquant) |
...
| `homing.backward` | Référencement arrière en cours |
...
| `simplePenetration.preflight` | Vérifications préalables |
| `strokeEngine` | Mode Stroke Engine |
| `strokeEngine.idle` | Stroke Engine inactif |
| `strokeEngine.preflight` | Vérifications préalables |
...
| `wifi` | Mode de configuration WiFi |
| `wifi.idle` | Configuration WiFi inactive |
| `help` | Écran d'aide |
| `help.idle` | Aide inactive |
| `error` | État d'erreur |
| `error.idle` | Erreur en état inactif |
...
| Propriétés | READ |
| But | Obtenir les motifs de course disponibles |
...
| Propriétés | READ, WRITE |
| But | Obtenir des descriptions pour des motifs de course individuels |

Pour récupérer une description de motif :

<Steps>
<Step title="Écrire l'index du motif">
...
| Motif | Indice | Description |
| ------- | ----- | ----------- |
| Simple Stroke | 0 | Accélération, roue libre et décélération également réparties ; aucune sensation |
| Teasing Pounding | 1 | Changements de vitesse avec sensation ; équilibre les mouvements plus rapides |
| Robo Stroke | 2 | La sensation fait varier l'accélération ; du robotique au progressif |
| Half'n'Half | 3 | Les courses en pleine et demi-profondeur alternent ; la sensation affecte la vitesse |
| Deeper | 4 | La profondeur de course augmente par cycle ; la sensation détermine le nombre |
| Stop'n'Go | 5 | Pauses entre les courses ; la sensation ajuste la longueur |
| Insist | 6 | Modifie la longueur, maintient la vitesse ; la sensation influence la direction |
...
| Position | 0-100 (pourcentage de la course) |
...
<Tip>
Pour la lecture de funscript, consultez l'outil [Lecteur Funscript](/ossm/software/communication/ble#commandes-de-streaming-expérimental) qui gère automatiquement le timing et la génération de commandes.
</Tip>
...
<Info>
Pour une documentation GPIO détaillée, y compris des exemples d'intégration matérielle, voir [Contrôle GPIO](/ossm/software/communication/gpio).
</Info>
...
| UUID de la caractéristique | `0000ffe1-0000-1000-8000-00805f9b34fb` |
| Propriétés | READ, WRITE, NOTIFY, INDICATE |
...
| 2 | Octet de poids faible du temps | LSB du temps en ms |
...
| Caractéristique | UUID | Valeur |
| -------------- | ---- | ----- |
| Service | `180A` | Service d'informations sur les appareils |
| Nom du fabricant | `2A29` | "Research And Desire" |
...
| Plage | Plage hexadécimale | Description |
| ----- | --------- | ----------- |
...
| 0x3 | 0x3000–0x3FFF | Informations sur les motifs |
...
| UUID des services | Service principal + Service d'informations sur les appareils |
| Intervalle de diffusion | 20-40 ms (optimisé pour la fiabilité) |
| Redémarrage automatique | La diffusion reprend lorsque tous les clients se déconnectent |
...
| Cryptage | BLE Secure Connections activées |
| Bonding | Désactivé (pas de couplage persistant) |
...
<Step title="Découvrez les services">
...
**Symptômes** : la caractéristique d'état n'est jamais mise à jour après l'abonnement.

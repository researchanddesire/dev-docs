...
Building a product for other R+D users? Start with [Build an app](/build-an-app) for developer access, registration, and platform-specific examples.

All API requests require a valid token passed in the `Authorization` header. Personal API tokens are opaque credentials associated with your account. Third-party products can use [OAuth account connections](/dashboard/api/oauth) when enabled for their environment.

<Info>
  Creating API tokens requires an **Ultra** subscription. If you don't have one
  yet, visit
  [Subscription](https://dashboard.researchanddesire.com/app/settings/subscription)
  to upgrade.
</Info>
...
<Tip>Revoke and recreate tokens periodically as a security best practice.</Tip>

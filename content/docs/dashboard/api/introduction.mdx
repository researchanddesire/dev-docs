...
<Info>
  Creating **personal API tokens** requires an Ultra subscription. OAuth
  developer applications have a separate approval process; existing product
  restrictions still apply. [Learn about personal
  tokens](/dashboard/api/authentication).
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Endpoint Reference" icon="code" href="/api-reference">
...
Requests are rate limited **per API token**, at a default of **60 requests per
minute**. When you exceed the limit, the API responds with `429` and the
...
<Note>
  If you receive a `429`, wait for the `Retry-After` duration before retrying —
  requests sent before then will keep returning `429` until the window resets.
</Note>

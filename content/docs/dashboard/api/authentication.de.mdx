...
Entwickeln Sie ein Produkt für andere R+D-Nutzer? Beginnen Sie mit [Eine App entwickeln](/de/build-an-app) für Entwicklerzugang, Registrierung und Plattformbeispiele.

Alle API-Anfragen benötigen ein gültiges Token im `Authorization`-Header. Persönliche API-Token sind undurchsichtige Zugangsdaten für Ihr Konto. Drittanbieter-Produkte können [OAuth-Verbindungen](/de/dashboard/api/oauth) verwenden, wenn diese in ihrer Umgebung aktiviert sind.

<Info>
  Für die Erstellung von API-Tokens ist ein **Ultra**-Abonnement erforderlich.
  Falls Sie noch keins haben, besuchen Sie
...
<Step title="Auf Token erstellen klicken">
  Klicken Sie auf die Schaltfläche **Token erstellen** und geben Sie Ihrem Token
  einen aussagekräftigen Namen (z. B. „Home Assistant Integration“ oder „Python
  Script“).
</Step>
...
<Tip>
  Widerrufen und erstellen Sie Tokens regelmäßig neu – eine bewährte
  Sicherheitsmaßnahme.
</Tip>

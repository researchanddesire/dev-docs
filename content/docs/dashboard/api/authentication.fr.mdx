---
title: "Authentification"
description: "Créez et gérez des jetons API pour authentifier vos requêtes"
---

Vous créez un produit pour d’autres utilisateurs R+D ? Commencez par [Créer une application](/fr/build-an-app) pour l’accès développeur, l’enregistrement et les exemples par plateforme.

Toutes les requêtes API nécessitent un jeton valide dans l’en-tête `Authorization`. Les jetons personnels sont des identifiants opaques associés à votre compte. Les produits tiers peuvent utiliser les [connexions OAuth](/fr/dashboard/api/oauth) lorsqu’elles sont activées dans leur environnement.

<Info>
  La création de jetons API nécessite un abonnement **Ultra**. Si vous n'en avez
  pas encore, consultez
  [Abonnement](https://dashboard.researchanddesire.com/app/settings/subscription)
  pour passer à une formule supérieure.
</Info>

## Créer un jeton

<Steps>
  <Step title="Accéder aux clés API">
    Accédez à **Paramètres > Clés API** dans la barre latérale du tableau de bord.
  </Step>

<Step title="Cliquer sur Créer un jeton">
  Cliquez sur le bouton **Créer un jeton** et donnez un nom descriptif à votre
  jeton (par exemple, « Home Assistant Integration » ou « Python Script »).
</Step>

  <Step title="Copier votre jeton">
    Votre jeton sera affiché exactement une fois. Copiez-le et conservez-le en toute sécurité.

    <Warning>
    Vous ne pourrez plus revoir ce jeton. Si vous le perdez, vous devrez en créer un nouveau.
    </Warning>

  </Step>
</Steps>

## Utiliser votre jeton

Incluez le jeton comme jeton Bearer dans l'en-tête `Authorization` :

<RequestExample>
```bash cURL
curl -X GET \
  https://dashboard.researchanddesire.com/api/v1/users \
  -H "Authorization: Bearer YOUR_API_TOKEN"
```

```python Python
import requests

headers = {"Authorization": "Bearer YOUR_API_TOKEN"}
response = requests.get(
    "https://dashboard.researchanddesire.com/api/v1/users",
    headers=headers,
)
data = response.json()
```

```javascript JavaScript
const response = await fetch(
  "https://dashboard.researchanddesire.com/api/v1/users",
  {
    headers: { Authorization: "Bearer YOUR_API_TOKEN" },
  },
);
const data = await response.json();
```

</RequestExample>

## Gérer les jetons

### Afficher les jetons

La page des paramètres **Clés API** affiche tous vos jetons actifs :

- **Nom** : le libellé que vous avez attribué
- **Créé** : la date de création du jeton
- **Dernière utilisation** : la dernière fois que le jeton a été utilisé pour effectuer une requête

### Révoquer les jetons

Cliquez sur l'icône de la corbeille à côté d'un jeton pour le révoquer. La révocation est immédiate : les requêtes qui utilisent ce jeton reçoivent immédiatement des réponses `401 Unauthorized`.

<Tip>
  Révoquez et recréez régulièrement vos jetons, conformément aux bonnes
  pratiques de sécurité.
</Tip>

### Limites des jetons

Vous pouvez avoir jusqu'à **5 jetons actifs** à la fois. Si vous en avez besoin de davantage, révoquez d'abord un jeton inutilisé.

## Réponses aux erreurs

| Statut | Signification                                                                     |
| ------ | --------------------------------------------------------------------------------- |
| `401`  | Jeton manquant, invalide, expiré ou révoqué                                       |
| `403`  | Le jeton est valide, mais vous n'avez pas l'autorisation d'effectuer cette action |
| `404`  | Ressource introuvable (ou inaccessible avec vos autorisations)                    |

<ResponseExample>
```json 401 Unauthorized
{
  "ok": false,
  "error": "Missing or invalid Authorization header"
}
```

```json 401 Token Revoked
{
  "ok": false,
  "error": "Token has been revoked"
}
```

</ResponseExample>

...
| --- | --- | --- |
| Assembly guide, BOM page prose, assembly images | `assembly-docs/` in the product repo | `assembly-docs` |
...
- `assembly-docs/` starter pages with `nav.yml` and generated BOM markers.
- `developer-docs/docs/` starter pages with `.pages`.
- `hardware/bom.csv`, `hardware/cad/`, and `hardware/pcb/` starters.
...
2. Replace `PRODUCT_NAME`, `PRODUCT_REPO`, `PRODUCT_SLUG`, and
   `PRODUCT_LICENSE`.
3. Add the new repo to `assembly-docs/scripts/assemble-docs.sh` and
   `dev-docs/scripts/assemble-docs.sh`.
4. Add navigation placeholders or top-level entries in the aggregator repos.
5. Configure `DOCS_DISPATCH_TOKEN` in the product repo.
